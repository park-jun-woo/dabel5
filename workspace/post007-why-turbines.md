# 우주에서 태양광 패널 대신 터빈을 쓰는 이유 — 자기복제의 관점

> Reddit 글 7 초안 (r/IsaacArthur)
> 확정 제목: `Solar panels and turbines are both 30% efficient in space — but only one fits a self-replicating loop`
> 선행 글: post006-power-transmission-local.md
> 트리거: "왜 PV 안 쓰고 구닥다리 터빈이야?" 설계 심화
...
Here's the thing nobody mentions about Dyson swarm power: solar panels and solar thermal turbines are both ~30% efficient. Efficiency is a wash.

The difference is everything else:
- PV throws away 70% as low-grade heat. Turbines cascade it through smelting → manufacturing → habitats → data centers.
- PV requires gallium arsenide or semiconductor-grade silicon — neither available from asteroids. Turbines are made from Fe-Ni and Ti — already in the supply chain.
- PV degrades from radiation and can't be replaced locally. Turbine blades can be recast on-site.

In a self-replicating system, "can you make it from what you have?" is the only question that matters. Turbines pass. PV doesn't.

But turbines have a problem PV doesn't: you have to dump the waste heat, and in space, there's no air to help. Next post: how do you radiate away 855 MW in vacuum?
